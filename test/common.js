...
import CID as OldCID from 'cids'
...
const { multihash, multibase, CID } = multiformats
const { DAGNode, DAGLink, util: pbUtil } = IpldDagPb

multibase.add(base58)

const unknownCid = CID.from('bafyreihyrpefhacm6kkp4ql6j6udakdit7g3dmkzfriqfykhjw6cad5lrm')
...
async function toPbBlock (pnd) {
  ...
  const cid = CID.from((await pbUtil.cid(binary, { cidVersion: 0 })).toString())
  return { cid, binary }
}

async function toCborBlock (cb) {
  const binary = await multiformats.encode(cb, 'dag-cbor')
  const mh = await multihash.hash(binary, 'sha2-256')
  const cid = CID.create(1, multiformats.get('dag-cbor').code, mh)
  return { cid, binary }
}

async function toRawBlock (binary) {
  const mh = await multihash.hash(binary, 'sha2-256')
  const cid = CID.create(1, multiformats.get('raw').code, mh)
  return { cid, binary }
}
...
async function makeData () {
  if (!rawBlocks) {
    ...
    const pnd1 = new DAGNode(null, [
      new DAGLink('cat', rawBlocks[0].binary.byteLength, new OldCID(rawBlocks[0].cid.toString()))
    ])
    pbBlocks.push(await toPbBlock(pnd1))

    const pnd2 = new DAGNode(null, [
      new DAGLink('dog', rawBlocks[1].binary.byteLength, new OldCID(rawBlocks[1].cid.toString())),
      new DAGLink('first', pnd1.size, new OldCID(pbBlocks[0].cid.toString()).toString())
    ])
    pbBlocks.push(await toPbBlock(pnd2))

    const pnd3 = new DAGNode(null, [
      new DAGLink('bear', rawBlocks[2].binary.byteLength, new OldCID(rawBlocks[2].cid.toString())),
      new DAGLink('second', pnd2.size, new OldCID(pbBlocks[1].cid.toString()))
    ])
