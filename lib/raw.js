...
async function readRaw (fd, blockIndex) {
  ...
  const binary = Buffer.alloc(blockIndex.length)
  let read
  if (typeof fd === 'number') {
    read = (await fs.read(fd, binary, 0, blockIndex.length, blockIndex.offset)).bytesRead
  } else if (typeof fd === 'object' && typeof fd.read === 'function') {
    read = (await fd.read(binary, 0, blockIndex.length, blockIndex.offset)).bytesRead
  } else {
