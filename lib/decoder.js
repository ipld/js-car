...
import { StoreReader, StoreIndexer } from './storage.js'
...
async function indexerComplete (Block, reader) {
  const decoder = createDecoder(Block, reader)
  const { version, roots } = (await decoder.header()).decode()
  const indexData = []
...
export { decodeComplete, indexerComplete, bytesReader, asyncIterableReader }
