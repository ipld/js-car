...
import { StoreReader, StoreIterator, StoreIndexer } from './storage.js'
...
function createDecoder (Block, reader) {
...
async function decodeReaderComplete (Block, reader) {
  const decoder = createDecoder(Block, reader)
...
async function decodeIterator (Block, reader) {
  const decoder = createDecoder(Block, reader)
  const { version, roots } = (await decoder.header()).decode()
  return new StoreIterator(version, roots, decoder.blocks())
}

async function decodeIndexerComplete (Block, reader) {
  const decoder = createDecoder(Block, reader)
  const { version, roots } = (await decoder.header()).decode()
  const indexData = []
  ...
  return new StoreIndexer(version, roots, indexData)
}
